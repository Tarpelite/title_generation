...
@dataclass
class DataCollatorForGAN(DataCollator):
...
@dataclass
class DataCollatorForMixLM(DataCollator):
    tokenizer:PreTrainedTokenizer
    mlm:bool=True
    mlm_probability: float = 0.15
    ...
    def collate_batch(self, examples:List) -> Dict[str, torch.Tensor]:
        ...
        all_domain_labels = torch.tensor([x[1] for x in examples], dtype=torch.long)

        inputs, labels = self.mask_tokens(all_input_ids)

        return {
            "input_ids":inputs,
            "labels": labels,
            "next_sentence_label":all_domain_labels
        }
