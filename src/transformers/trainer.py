...
class GANTrainer:
    """
    GANTrainer is a simple but feature-complete training and eval loop for PyTorch,
    optimized for Transformers.
    """
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if steps_trained_in_current_epoch > 0:
                    steps_trained_in_current_epoch -= 1
                    continue
                loss = self._training_step(generator, discriminator, inputs, optimizer)
                tr_loss += loss
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.fp16:
                        ...
                        torch.nn.utils.clip_grad_norm_(generator.parameters(), self.args.max_grad_norm)
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        ...
                        if hasattr(generator, "module"):
                            assert generator.module is self.generator
                        else:
                            assert generator is self.generator
                        # Save model checkpoint
                        output_dir = os.path.join(self.args.output_dir, f"{PREFIX_CHECKPOINT_DIR}-{self.global_step}")
    ...
    def _training_step(
        self, generator: nn.Module, discriminator: nn.Module, inputs: Dict[str, torch.Tensor], optimizer: torch.optim.Optimizer
    ) -> float:
        generator.train()
        discriminator.train() # learning rate is zero
    ...
    def _save(self, output_dir: Optional[str] = None):
        ...
        # Save a trained model and configuration using `save_pretrained()`.
        # They can then be reloaded using `from_pretrained()`
        if not isinstance(self.generator, PreTrainedModel):
            raise ValueError("Trainer.model appears to not be a PreTrainedModel")
